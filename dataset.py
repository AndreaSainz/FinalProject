...
class LoDoPaBDataset(Dataset):
    ...
    def __init__(self, ground_truth_dir, n_single_BP=16):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        angles_SBP = np.linspace(0, len(angles) - 1, self.n_single_BP, dtype=int)

        for angle_SBP in angles_SBP:
          
          # Define Fan Beam Geometry for each angle
          pg_SP = ts.cone(angles = angles[angle_SBP], cone_angle = self.cone_angle, shape=(1, self.num_detectors))
          # Compute Back Projection
          A_SP = ts.operator(vg,pg_SP)
          ...
          single_back_projection.append(torch.tensor(back_projection_SP.numpy()))
        ...
        return {'ground_truth': sample_slice, 'sinogram': sinogram, 'backprojection': back_projection, 'single_back_projections': single_back_projection}